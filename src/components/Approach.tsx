      ...
      <div className="w-[min(calc(100%-2rem),var(--max))] mx-auto">
        <FadeUp>
          ...
          <p className="m-0 max-w-[44rem] text-[var(--muted)] text-base">
            ...
            help a person, a workflow, or a business. The goal is not more complexity. The goal is more clarity, better
        ...
        <FadeUp>
          <ol
            className="card-soft mt-10 m-0 list-none rounded-[var(--radius-xl)] overflow-hidden divide-y divide-[var(--border-warm)] p-0"
            aria-label="Three approach principles"
          >
            {principles.map(({ number, icon: Icon, title, description }) => (
              <li
                key={number}
                className="grid grid-cols-[52px_1fr] gap-[0.85rem] items-start py-6 px-4 sm:px-5"
              >
                <div className="flex flex-col items-center gap-1">
                  <div className="w-9 h-9 rounded-full bg-[var(--accent-soft)] text-[var(--accent)] grid place-items-center font-bold text-sm">
                    {number}
                  </div>
                  <div className="grid h-8 w-8 place-items-center rounded-lg border border-[var(--border-warm)] bg-[var(--surface)] text-[var(--accent)]">
